...
- (void)audioSessionChangeObserver:(NSNotification *)notification{
    ...
    if (audioSessionRouteChangeReason == AVAudioSessionRouteChangeReasonOldDeviceUnavailable){
        if (_typeHeadphone != @"KeepPlay" && player) {
            [player pause];
        }
    }
...
RCT_EXPORT_METHOD(setHeadphone:(NSString *)typeName) {
    if ([typeName isEqual: @"KeepPlay"]) {
        _typeHeadphone = @"KeepPlay";
    } else {
        _typeHeadphone = @"Pause";
    }
}
